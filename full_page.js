...
document.addEventListener('DOMContentLoaded', () => {
  console.log("RatScale DOM loaded");
  ...
  if (browserDetails) {
    browserDetails.textContent = `Browser: ${getBrowserInfo()} | Extension version: ${chrome.runtime.getManifest().version}`;
  }
    ...
    try {
      ...
      selectedPlatforms.forEach(platform => {
        ...
        platformFolder.file("README.txt", `${platform.toUpperCase()} Icons\n` +
                          `Generated using RatScale by RatLabs\n` +
      ...
      const readmeContent = `RatScale Icon Pack\n` +
  ...
  function triggerDownload() {
    ...
    chrome.downloads.download({
      ...
      if (chrome.runtime.lastError) {
        ...
        updateStatus('Error saving file: ' + chrome.runtime.lastError.message, '#ff5555');
      } else {
  ...
  function showAfterTaskOptions() {
    ...
    mainMenuButton.addEventListener('click', () => {
      ...
      resetTool();
      // Close this window - which will take the user back to the browser action popup
